...
export default function Home() {
    ...
    const getMyWeb = async () => {
        try {
            setLoading(true);
            const body = { id: params.id };

            const res = await fetch("https://admirable-croissant-3aaba1.netlify.app/api/getWebId", {
                method: "POST",
                headers: {
    ...
    const update = () => {
        if (key === "contactDatacoordinate" && (edit.split(",").length !== 2 || isNaN(Number(edit.split(",")[0])) || isNaN(Number(edit.split(",")[1])))) {
            alert("Input a correct coordinate separated by comma")
            return;
        }
    ...
    }, [textsToSave, imagesToSave, email])
            ...
            <Suspense fallback={<Loader />}>
                ...
                {modal &&
                    <div onClick={(e) => handleParentClick(e)} className="fixed flex items-center justify-center top-0 w-full min-h-screen bg-[#00000023] z-[999999]">
                        <form className="bg-white relative z-auto flex flex-col w-full md:w-1/2 xl:w-1/3 m-auto rounded-lg p-9">
                            ...
                            <button className="bg-blue-600 text-white mt-5 w-fit px-9 text-xs ml-auto py-2 rounded" onClick={update}>OK</button>
                        </form>
                    </div>
                }
                <div onClick={(e) => handleParentClick(e)} className={`fixed flex items-center transition-opacity duration-700 justify-center top-0 w-full min-h-screen bg-[#00000023] ${!production && emailModal ? "z-[999999] opacity-100" : "-z-[91] opacity-0"}`}>
                    <form className="bg-white relative z-auto flex flex-col w-full md:w-1/2 xl:w-1/3 m-auto rounded-lg p-9">
                        ...
                        <button className="bg-blue-600 text-white mt-5 w-fit px-9 text-xs ml-auto py-2 rounded" onClick={(e) => mailSetter(e)}>OK</button>
                    </form>
                </div>
                {imageModal &&
                    <div onClick={(e) => handleParentClick(e)} className="fixed flex items-center justify-center top-0 w-full min-h-screen bg-[#00000023] z-[999999]">
                        <form className="bg-white relative z-auto flex flex-col w-full md:w-1/2 xl:w-1/3 m-auto rounded-lg p-9">
                            ...
                            <button className="bg-blue-600 text-white mt-5 w-fit px-9 text-xs ml-auto py-2 rounded" onClick={() => setImageModal(false)}>OK</button>
                        </form>
                    </div>
                }
            </Suspense>
